...
def create_remote_file_bundle(name):
    ...
    with api.Bundle(TEST_CONTEXT, name=name) as b:
        ...
        s3_path_3 = os.path.join(
            b.get_remote_directory("vince/klartho"), "test_s3_file.txt"
        )
...
def test_fast_push():
    ...
    for i in range(3):
        ...
        bundles[
            name
        ].push()  # need this to put the bundles in the remote b/c of moto mp issues
...
def X_test_api_push_delocalize():
    ...
    api.push(
        TEST_CONTEXT, delocalize=True
    )  # push and remote all data, then pull and localize.
...
def test_bundle_link_localization_with_remote():
    ...
    for i, f in enumerate(b.data):
        assert b.data[i].startswith(
            "s3://"
        )  # testing if we can both s3 and local paths in a link frame
        b.localize(f)
        assert b.data[i].startswith("/")
