...
# ...and install Disdat
RUN $KICKSTART_ROOT/bin/kickstart-python.sh -n $VIRTUAL_ENV $BUILD_ROOT/disdat/setup.py
RUN $KICKSTART_ROOT/bin/install-python-package-from-source-tree.sh $VIRTUAL_ENV $BUILD_ROOT/disdat
